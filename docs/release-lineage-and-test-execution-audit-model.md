# Release lineage, iterations, and test-execution audit model
...
- which controlled implementation/test iterations occurred and why;
- how many genuine test executions ran within each iteration and stage;
- which executions failed, passed, were skipped, or remain unresolved;
- which evidence was produced by each execution;
...
- evidence can carry `sdlcStage` and `testExecutionId`;
- the portal groups evidence by `test_execution_id`;
- execution labels are numbered by chronology;
...
However, the portal currently derives an execution from the presence of evidence carrying the same string identifier. It also lacks a first-class record for controlled rework iterations. This creates five material limitations:
...
3. Global labels such as `Execution 14/16` show chronology but become difficult to read when the list is grouped by stage.
...
5. A retry, remediation pass, and unrelated workflow run can be displayed as equivalent "cycles", obscuring why development returned to an earlier step.

REQ-093 demonstrates the result. The chronological numbering logic can correctly identify the earliest and latest evidence groups, but the reviewer sees non-sequential labels inside stage-grouped sections. Some Stage 3 evidence-pack uploads are counted as executions, while several Stage 5 groups have no authoritative outcome metadata. The result is technically grouped evidence, not yet a complete test-execution and release-lineage model.

## Design principles

1. **A release is not an iteration or execution.** A release is a governed scope of change.
2. **An iteration is controlled rework.** It begins only when a failed execution, defect, missing acceptance criterion, approved scope correction, or review rejection sends work back through implementation and verification.
3. **A test execution is one governed run.** Retries and reruns are separate executions within the same iteration unless they introduce new rework.
4. **An evidence upload is not an execution.** Documents can describe a release, stage, iteration, or execution, but uploading a document proves none of them occurred.
5. **Bundling preserves identity.** A predecessor remains queryable under its original release identifier, status, stage history, iterations, and evidence.
6. **Superseded does not mean abandoned.** The record must identify the successor, reason, absorbed scope, and final disposition.
7. **Evidence is referenced, not reassigned.** Evidence remains owned by the source release and execution that produced it. A successor approval may inherit it through an explicit relationship.
8. **Stage counts and chronology are separate views.** Stage grouping answers how many attempts a stage needed. A timeline answers how the overall release evolved.
9. **Outcomes are producer facts.** The portal must not infer pass or fail from filenames, artifact presence, or display order when an authoritative workflow result is available.
10. **Final approval freezes scope.** The release snapshot records the exact constituent releases, iterations, executions, evidence, commits, and approvals that were reviewed.
11. **No silent branch-window attribution.** A later REQ must not absorb every earlier row merely because it is the next tracked release on the branch. The producer must submit explicit bundle membership or the portal must flag the scope for reconciliation.
...
| Release iteration      | A controlled pass through implementation and verification caused by a recorded trigger. An iteration may contain multiple test executions. |
| Test execution         | A real execution attempt at one stage for one source release.                                                                              |
...
While work is ongoing, the same area must show the current stage, current blocker, latest completed execution, and next required action. It must never imply that CI is green while a required workflow is queued or running.
...
| -------------------- | -------: | --------------------------: | -------: | ------------------------: | ----------: | ----------------- |
| REQ-090              | Complete |   Iteration 1: 3 executions | Complete |               1 execution |     Not run | Superseded        |
| Housekeeping release |      N/A |   Iteration 1: 2 executions |  Partial |                   Not run |     Not run | Absorbed          |
| Follow-up fix        | Complete |   Iteration 2: 2 executions | Complete |                   Not run |     Not run | Absorbed          |
| REQ-093              | Complete | 2 iterations / 3 executions | Complete | Iteration 2: 2 executions | 1 execution | Released          |

The UI must distinguish `Not run`, `Not applicable`, `Inherited`, `Incomplete`, and `0 recorded executions`. These states are not interchangeable.

### 4. Stage-scoped execution labels

Executions are numbered within their source release and stage:

```text
REQ-093 - Iteration 2 - Implement/test - Execution 3 of 3 - Passed
REQ-093 - Iteration 2 - UAT - Execution 2 of 2 - Passed
REQ-090 - Iteration 1 - UAT - Execution 1 of 1 - Passed - Inherited by REQ-093
```

The portal may also display the external run identifier, but it must not use that identifier as the human sequence number. Global `Execution N/total` labels must not be the primary label inside stage-grouped views.

Each execution expands to show:

- start and completion time;
- workflow, job, run ID, and run attempt;
- source branch and commit SHA;
- iteration number and the event that opened that iteration;
- environment;
- authoritative outcome and conclusion;
- failure or skip reason;
- linked incident and remediation;
- superseding/retry execution where applicable; and
...
6. CI passed
7. UAT failed, was remediated, and passed on Execution 2
...
Release tickets, RTMs, plans, bundled-change manifests, security summaries, and AI-use notes are not executions. Screenshots, gate outcomes, coverage, E2E reports, test results, and smoke results can be execution-scoped.
...
- execution identifiers, order, and outcomes;
...
One row per controlled implementation and verification pass:

| Field                              | Purpose                                                                                                             |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `id`                               | Stable UUID.                                                                                                        |
| `project_id` / `source_release_id` | Owning project and release.                                                                                         |
| `iteration_number`                 | Monotonic ordinal within the source release.                                                                        |
| `opened_at` / `closed_at`          | Iteration chronology.                                                                                               |
| `trigger_type`                     | `initial`, `failed_execution`, `defect`, `missing_acceptance_criterion`, `scope_correction`, or `review_rejection`. |
| `trigger_reference` / `reason`     | Linked incident, issue, review, or other auditable explanation.                                                     |
| `status`                           | `active`, `verified`, `superseded`, or `cancelled`.                                                                 |
| `commit_from` / `commit_to`        | Bounded change range for the rework pass.                                                                           |
| `opened_by` / `closed_by`          | Actor or trusted automation identity.                                                                               |

The initial implementation and verification pass is Iteration 1. A workflow
rerun, provider retry, or repeated test on the same build creates another test
execution inside the current iteration. It does not create an iteration.
Opening Iteration N+1 requires a recorded trigger and a change or approved
scope correction.
...
| `release_iteration_id`           | Controlled iteration containing this execution.                                                   |
...
The stage-scoped sequence is derived deterministically from `started_at`, then `idempotency_key`, within `(source_release_id, release_iteration_id, sdlc_stage)`. It is frozen in the final audit snapshot. A late historical event may alter an in-progress display ordinal but must emit a reconciliation audit event.
...
- `release.superseded`;
- `release_iteration.opened`;
- `release_iteration.closed`;
- `release_iteration.reconciled`;
- `test_execution.started`;
- `test_execution.completed`;
- `test_execution.reconciled`;
- `evidence.inherited`;
...
The workflow must create/update the execution independently of evidence upload. Completion reporting must run under `if: always()` so a failed execution is recorded even when artifact generation fails.
...
1. Add the execution-event, evidence-scope, and bundle-manifest schemas to the installer.
...
4. Define the cutover requirement: the portal API must be deployed before generated producers emit the new contract.
5. Update evidence-tier documentation so document uploads cannot be classified as test executions.

### Phase I2 - workflow and CLI production

1. Extend the CLI and `upload-evidence.sh` with test execution lifecycle operations or a dedicated command such as `devaudit execution start|complete`.
...
5. Stop applying `--test-execution` to release-level document uploads.
6. Remove generated legacy cycle terminology and grouping-only payloads at cutover; no generated-project backwards-compatibility mode is required.
...
3. Generate execution tables per constituent release and stage in `test-execution-summary.md`.
...
1. Add `release_bundle_memberships`, `release_iterations`, test execution records, and approval snapshot storage.
...
4. Retain `bundle_context` and `superseded_by_release_id` only as portal migration inputs; generated producer artefacts use the canonical iteration and test-execution contract.
5. Add repository and service methods with project and organisation ownership checks.

### Phase P2 - ingestion APIs

1. Add authenticated, idempotent execution start/upsert/complete endpoints.
...
5. Reject cross-project links, self-supersession, executions without a source release, and evidence/execution ownership mismatches.
6. Emit audit events for every lifecycle and reconciliation mutation.
7. Keep legacy portal records readable during migration without preserving legacy terminology or payload production in generated projects.
...
- stage completion and execution counts per constituent;
- execution outcomes and links;
- evidence grouped by owner/scope/stage/execution;
- inherited evidence relationships;
...
1. Replace the global execution label as the primary label with release/stage-scoped numbering.
2. Add the constituent release matrix and lineage navigation.
3. Add stage filters and a chronological timeline toggle.
4. Show first/latest markers and the current active execution while a release is in progress.
5. Separate release documents, stage documents, and execution evidence.
...
10. Render the same lineage and execution model in shared reviewer links and audit-pack exports.

### Phase P5 - readiness and approval rules

1. Compute readiness from authoritative executions and required checks, not artifact quantity.
2. Prevent approval while a required check is queued, running, action-required, failed, cancelled, or unknown.
3. Require every failed required execution to have a later passing execution, accepted exception, or explicit supersession disposition.
4. Require bundle membership and evidence inheritance conflicts to be resolved before approval.
5. Require the final production execution, deployment provenance, and smoke result before `released`.
...
- stage-scoped execution numbering and deterministic chronology;
- late-arriving execution events and reconciliation audit events;
- executions with no evidence and evidence with no execution;
...
1. Dry-run the repair and report proposed releases, memberships, executions, excluded document-only groups, outcomes, and confidence.
...
7. Keep document-only groups as release/stage evidence and remove them from displayed execution totals.
...
3. Release the installer with the canonical iteration, execution, and bundle contracts.
...
10. Remove legacy portal derivation after repaired records have been verified; generated projects do not retain a legacy emission mode.
...
Existing documents `docs/issues/test-executions-producer.md` and `docs/issues/test-executions-portal.md` describe the original evidence-grouping implementation. Their statement that no dedicated execution entity is required is superseded by this design because the reviewer requirement now includes test execution lifecycle, failed executions with no artifacts, stage-specific counts, release ownership, and immutable audit snapshots.

## Acceptance criteria

The work is complete only when all of the following are true:

1. A reviewer can see execution counts per SDLC stage and constituent release without counting artifacts.
2. An execution label identifies source release, iteration, stage, ordinal, total stage executions, and outcome.
3. The timeline clearly identifies the first and latest events and explains every retry or failure.
4. Document uploads do not increase execution counts.
...
8. Inherited evidence shows its source release, source stage/execution, and reason for inclusion.
9. Unknown, queued, and running required checks cannot be represented as green or complete.
10. Production release requires deployment provenance and a passing or formally excepted production verification execution.
...
14. A retry remains in the current iteration, while rework opens a new iteration with a recorded trigger and bounded change scope.
...
For an in-progress release, the portal shows the current approval envelope, its preserved constituent releases, the stage currently running, completed and unresolved executions, required checks still in progress, and the exact next gate.

For a completed release, the portal shows an immutable journey from original scope through every predecessor, stage, execution, incident, remediation, approval, deployment, and production verification. A reviewer can understand what happened quickly; an auditor can follow every summary value back to the original release, execution, evidence, actor, and timestamp.
