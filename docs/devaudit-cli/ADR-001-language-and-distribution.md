...
2. **Plugin system fits naturally.** Plugins are npm packages with TypeScript types from `@metasession-dev/devaudit-plugin-sdk`. A plugin author who already writes TypeScript for their app gets immediate productivity. Same model for org-private plugins via private npm registries or Git URLs.
...
- npm publishes both unscoped `devaudit` (binary path: `node_modules/devaudit/bin/devaudit.js`) and scoped `@metasession-dev/devaudit-cli` (alias) — npm convention.

### Plugin distribution

- First-party plugins (`devaudit-plugin-prisma`, etc.) ship as separate npm packages from the same monorepo.
- Plugin SDK (`@metasession-dev/devaudit-plugin-sdk`) ships as a standalone npm package — third parties depend on it.
