...
| **AI coding agent** | GitHub-side, plus one narrow portal exception | Claude Code, Cursor, Windsurf, etc. Has no credential of its own for anything it does under GitHub — it acts under the developer's already-authenticated git session; its contribution is attributed, not authenticated: a `Co-Authored-By` trailer, not a login. The one exception (devaudit#867): if the operator opted into `--with-viewer-key` at onboarding, the agent may hold a read-only `DEVAUDIT_VIEWER_API_KEY` to query release/check/cycle status directly — it can never upload evidence or approve a release with it, so this doesn't create a portal identity for the agent, only a read-only lookup capability. | none for GitHub · optionally `DEVAUDIT_VIEWER_API_KEY` (read-only) |
...
> This diagram predates the optional `DEVAUDIT_VIEWER_API_KEY` (devaudit#867, table below) and doesn't depict it — it's a read-only side-channel an agent may query at any point in the flow, not another hop in the release's own promotion path, so it doesn't change any node above.
...
| **`DEVAUDIT_VIEWER_API_KEY`** (optional, devaudit#867) | Minted by the portal during `devaudit install --with-viewer-key` (opt-in; off by default) | Written as a repo secret by `devaudit install`, as `DEVAUDIT_VIEWER_API_KEY` — or exported locally if the operator wants an agent to use it in an interactive session | **One project, read-only.** A compromised key can only query release/check/cycle status for that project — it cannot upload evidence, approve a release, or touch any other project. This is the one credential in this table safe to hand to an AI agent, because a leak or misuse can't mutate anything. | The read-back endpoints (`GET .../checks`, `GET .../cycles`, `GET .../resolve` release-lookup) — never a write route; `sdlc-implementer` or a CI status check, if configured to use it |
