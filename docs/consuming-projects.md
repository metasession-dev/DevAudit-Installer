...
**Resource owner must match the repo-owning account, not just any account/org you have access to.** devaudit-installer#670's root cause was a fine-grained PAT generated under a *different* organization than the one that actually owned the target repos — a fine-grained PAT's permissions are scoped to its resource owner, so it had zero access to the intended repos regardless of the permissions selected or how far out the expiration was set. Easy mistake to make in any multi-account/multi-org setup; double-check the resource owner before generating.
