...
- **W4 — Incident-report close-out hook** (`v0.1.31`, closes the automated arm of `ISO29119.3.5.4`, `SOC2.CC7.2`, `GDPR.Art-33`, `GDPR.Art-34`):
  `.github/workflows/incident-export.yml` fires on `issues: [closed]`, gated by `contains(labels.*.name, 'incident')`. The job calls `gh issue view --json` to capture title + body + comments + timeline + labels, renders to `compliance/governance/incident-report-<n>.md`, and routes based on the issue's `### Framework attribution` section (DevAudit-Installer#200 Fix 1):
  - **Path A (baseline-only):** If only `ISO29119.3.5.4` is ticked (no GDPR/SOC2/EUAIA), the report is **direct-committed to `develop`** — no PR, no REPLACE markers. GDPR triage is pre-filled as "N/A — no personal data scope". Severity is derived from issue labels. The next `compliance-evidence.yml` run uploads it as `incident_report` evidence.
  - **Path B (GDPR/SOC2/AI Act):** If any of `SOC2.CC7.2`, `GDPR.Art-33`, `GDPR.Art-34`, or `EUAIA.*` are ticked, a PR is opened on `chore/incident-export-<n>` with REPLACE markers for GDPR triage + sign-off (current behaviour preserved). **The PR still requires GDPR-triage and sign-off** before merge — auto-generated personal-data-Y/N decisions are not defensible.
  - **Missing attribution section** defaults to Path B (conservative — human review required).
  The operator-authored arm (W1's `incident-report.md` starter) remains the recommended default for very high-severity incidents where you want full editorial control.
