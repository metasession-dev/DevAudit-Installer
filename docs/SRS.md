...
| REQ-SKILL-E2E-011            | Phase 5½ evidence wiring validation: evidenceShot + @requirement + test title tags                   | Should   | `sdlc/files/_common/skills/e2e-test-engineer/SKILL.md`                                                                        |
...
- **Given** a spec file covering REQ-X **When** Phase 5½ checks **Then** it must carry `@requirement REQ-X`, at least one `evidenceShot()` per AC (for UI specs), and `[REQ-X]` in test titles/annotations. **Given** any check fails **Then** the skill halts with a gap report and does not proceed to Phase 6. **Given** an AC is API-only (no visual surface) **When** the user confirms the exemption **Then** the exemption is recorded in the test-execution-summary and the gate passes for that AC.
- **Error paths:** Missing `evidenceShot()` for a UI AC → halt; missing `@requirement` annotation → halt; missing `[REQ-XXX]` tag → halt. All must be fixed before Phase 6.
- **Fixtures/env:** A spec with all wiring correct (passes); a spec missing `evidenceShot()` for AC2 (halts); a spec missing `@requirement` (halts); an API-only spec with an explicit exemption (passes).
...
#### REQ-FRAMEWORK-CIYML-011 — Evidence-completeness gate: zero tagged tests AND zero screenshots fails the release
...
- **Source:** `sdlc/files/ci/ci.yml.template` `upload-evidence` job: after the screenshot loop, REQs in `ZERO_Screenshot_REQS` are checked against `ci-evidence/e2e-results.json` using a Python script that counts tests tagged with the REQ (via title `[REQ-XXX]` or `annotations`/`tags`). If `TAGGED_COUNT == 0`, `EVIDENCE_GAPS` increments. After all REQs are checked, if `EVIDENCE_GAPS > 0`, the step exits 1 with an `::error::` message.
...
- **Given** REQ-X has zero screenshots and the Playwright JSON has zero tests tagged with REQ-X **When** the evidence-completeness gate runs **Then** `EVIDENCE_GAPS` increments and the step exits 1 — the release PR turns red. **Given** REQ-X has zero screenshots but ≥1 tagged test **Then** the gate passes (non-blocking). **Given** `e2e-results.json` is missing **Then** `TAGGED_COUNT=0` (fails the gate if screenshots are also zero).
