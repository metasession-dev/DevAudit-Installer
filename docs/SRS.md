...
| REQ-SKILL-E2E-011            | Phase 5½ evidence wiring validation: evidenceShot + @requirement + tagTest + release/cycle provenance + .e2e-evidence-wired sentinel (#226) | Should | `sdlc/files/_common/skills/e2e-test-engineer/SKILL.md`                                                                        |
...
| REQ-FRAMEWORK-EVIDENCE-006   | E2E Regression evidence preserves source REQ, stage, cycle, and feature/regression provenance         | Must     | `sdlc/files/ci/compliance-evidence.yml.template`                                                                              |
...
#### REQ-CLI-PUSH-016 — `--test-execution` forwards `testExecutionId` form field for portal test execution grouping

- **Priority:** Should — enables the portal to group evidence by test execution (ISO/IEC/IEEE 29119-3 test execution logs / Test Status Reports); additive/optional, no regression without it (DevAudit-Installer#209).
- **Source:** `cli/src/commands/push.ts` (`PushOptions.testCycle`, forwarded as `testExecutionId` to `uploadEvidence`), `cli/src/lib/ci-upload.ts` (`UploadOptions.testExecutionId`, `buildUploadForm` sets `form.set('testExecutionId', opts.testExecutionId)`), `cli/src/index.ts` (`.option('--test-execution <id>', …)`).
- **Preconditions / inputs:** `--test-execution <id>` flag on the `push` command. The value is an opaque grouping key (typically a GitHub Actions `run_id`).
- **Given** `--test-execution 1234567` **When** push runs **Then** the multipart form additionally contains `testExecutionId=1234567`. **Given** no `--test-execution` flag **When** push runs **Then** the form omits `testExecutionId` entirely (the portal defaults to `null` — legacy/ungrouped). The field is orthogonal to `evidenceType`, `evidenceCategory`, and `sdlcStage`, and remains the compatibility fallback while first-class test execution APIs roll out.
...
- **Given** the installer repo **When** a developer or generated workflow needs canonical values for SDLC stage codes, cycle environments, cycle kinds, providers, outcomes, evidence scopes, bundle roles/relationships, or the idempotency-key shape **Then** those values are defined in the shared contract file and mirrored by the typed helper module. **Given** an older portal that only accepts `testExecutionId` **When** later producer code renders evidence-lineage fields **Then** `renderEvidenceLineageFields()` returns only `{ testExecutionId }`. **Given** a portal with first-class test execution APIs **When** the same helper renders the fields **Then** it emits `evidenceScope`, `testExecutionRecordId`, and preserves `testExecutionId` during dual-write rollout.
- **Error paths:** a `testExecutionRecordId` paired with a non-`cycle` scope throws in the helper; malformed idempotency keys fail the contract tests.
...
- **Source:** `sdlc/files/_common/3-compile-evidence.md` (Step 1a — Generate Test Execution Summary; Step 4a — Query portal for test execution data), `sdlc/files/_common/skills/sdlc-implementer/SKILL.md` (Phase 3 step 4a): the `test-execution-summary.md` template includes a `## Test Executions` section after `## Gate Results`, carrying a table of executions (CI Run, Gate Status, E2E Result, Coverage, Date) and a final assessment line. The `sdlc-implementer` skill populates this table by querying the portal API for all evidence records for this release grouped by `testExecutionId` (Step 4a) — the operator does not hand-assemble execution data. If the portal doesn't yet support `testExecutionId` grouping (pre-deployment of devaudit#535), the skill falls back to local execution records from `compliance/evidence/REQ-XXX/` artefact filenames and notes the fallback.
...
- **Given** a release with N completed test executions **When** the `sdlc-implementer` skill runs Phase 3 Step 4a **Then** it queries the portal API for evidence grouped by `testExecutionId` and populates the Test Executions table from the response — the operator does not manually assemble execution data. **Given** the portal does not yet support `testExecutionId` grouping **When** Step 4a runs **Then** the skill falls back to local execution records from artefact filenames and notes the fallback in the summary. **Given** a release with a single execution **When** Stage 3 generates the summary **Then** the Test Executions section lists that one cycle. **Given** a release where an execution failed **When** Stage 3 generates the summary **Then** the final assessment notes the failure and any outstanding incidents.
...
- **Fixtures/env:** A REQ with 2 completed CI runs (execution data from portal stub or local evidence directory); a REQ with a single execution; a REQ with a failed cycle.
...
- **Source:** `sdlc/files/ci/ci.yml.template` (`FLAGS` variable in `upload-evidence` job: `--test-execution ${{ github.run_id }}` appended alongside `--ci-run-id` and `--sdlc-stage`; also added to `FANOUT_FLAGS`), `sdlc/files/ci/compliance-evidence.yml.template` (same `FLAGS` addition), `scripts/upload-evidence.sh` (`--test-execution` flag parsed → `TEST_CYCLE` → forwarded as `testExecutionId` form field).
- **Preconditions / inputs:** CI run on a rendered consumer repo; `DEVAUDIT_BASE_URL` + `DEVAUDIT_API_KEY` configured.
- **Given** a CI run with `github.run_id=1234567` **When** the `upload-evidence` job runs **Then** every `upload-evidence.sh` invocation carries `--test-execution 1234567`, which forwards `testExecutionId=1234567` as a form field to the portal. **Given** the portal does not yet accept `testExecutionId` **When** an upload arrives **Then** the portal silently drops the unknown field (tolerant-read, same pattern as `sdlcStage`) — no error, no regression.
- **Error paths:** Missing `--test-execution` flag → `testExecutionId` omitted from the form; portal defaults to `null` (legacy/ungrouped).
- **Fixtures/env:** Rendered consumer repo; portal stub asserting `testExecutionId` form field on each upload; portal stub ignoring `testExecutionId` (tolerant-read).
...
- **Given** a release ticket, RTM, implementation plan, test plan, bundled-change manifest, test execution summary, security summary, or approval document **When** it is uploaded to the portal **Then** it is release-, stage-, or approval-scoped evidence, never execution-scoped evidence. **Given** a portal that has not yet rolled out first-class test execution lifecycle APIs **When** the generated workflows upload execution evidence **Then** they fail or leave the release unresolved rather than fabricating execution evidence. **Given** a portal with the new lifecycle APIs **When** later producer issues implement dual-write **Then** execution evidence adds `evidenceScope=execution` and `testExecutionRecordId` while preserving the first-class execution record.
...
- **Given** UAT fails **When** the command is run with `--outcome failed` **Then** a `--remediation-ref` is required by process and included as the incident/remediation reference in the completed cycle. **Given** a release is submitted with `submit-for-uat-review.sh` **Then** the script prints the UAT execution next step and must not record a passing Stage 4 execution itself.
...
#### REQ-FRAMEWORK-EVIDENCE-006 — E2E Regression evidence preserves source REQ, stage, cycle, and feature/regression provenance
...
- **Preconditions / inputs:** A consumer E2E workflow produces the configured result/report artifact and names the originating `REQ-XXX` in release-ticket scope or artifact metadata. The workflow has a CI run ID, source SHA, and an SDLC stage/cycle context.
...
- **Priority:** Should — the Test Completion Report is the encompassing artefact after all test executions; the Test Executions section is the cycle-aggregation view that distinguishes it from per-cycle logs (DevAudit-Installer#209, ISO/IEC/IEEE 29119-3 §4.14).
- **Source:** `sdlc/files/_common/3-compile-evidence.md` (Step 1a template): the `test-execution-summary.md` markdown template includes a `## Test Executions` section (table: Cycle | CI Run | Gate Status | E2E Result | Coverage | Date) followed by a `**Final assessment:**` line. The section is populated at Stage 3 by the `sdlc-implementer` skill (or operator) after all executions for a release are complete.
...
- **Fixtures/env:** A REQ with 2 completed CI runs; a REQ with a single execution; a REQ with a failed cycle (final assessment notes the failure).
...
| SRS-LIN-405-002 | Generated workflows shall start lifetest execution records before governed execution and complete them from the authoritative execution result, independently of evidence upload. |
