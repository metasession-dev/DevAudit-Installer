...
| REQ-FRAMEWORK-EVIDENCE-005   | Normal housekeeping remains integration history; standalone promotion is an explicit, validated exception                                                   | Must     | `sdlc/files/ci/compliance-evidence.yml.template`, `sdlc/files/_common/scripts/standalone-housekeeping-release.sh`             |
...
- **Given** normal housekeeping is merged after terminal-green PR review **When** its develop workflow runs **Then** any bare-date record is integration/history only: no release-ticket/security-summary stub PR is generated, no UAT or production approval is requested, and the work waits to be represented by the next tracked REQ's bundled-change lineage. **Given** an operator explicitly requests standalone housekeeping promotion **When** the declaration validates and production post-deploy completes **Then** the portal row is promoted to `standalone_housekeeping`; it remains distinct from normal integration history. **Given** the declaration is absent, malformed, or mismatched **Then** the standalone path fails rather than silently treating normal housekeeping as approval-ready.
