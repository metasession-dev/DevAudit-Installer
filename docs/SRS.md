...
- **Preconditions / inputs:** `DEVAUDIT_BASE_URL` and `DEVAUDIT_API_KEY` are set; `scripts/report-test-cycle.sh` is present; a tracked release exists in the portal.
- **Given** a reviewer has performed UAT **When** `scripts/record-uat-execution.sh --project-slug <slug> --release REQ-XXX --outcome passed --executor <identity> --tested-sha <sha> ...` runs **Then** it calls `scripts/report-test-cycle.sh start` and `complete` with `--sdlc-stage 4`, `--environment uat`, `--cycle-kind uat`, `--provider manual_uat`, a stable manual execution id, the tested SHA, reviewer identity, checklist/evidence references, and a terminal outcome. **When** the same command is re-run for the same release/SHA/outcome/refs **Then** the idempotency key is unchanged and the same UAT execution record is updated rather than duplicated.
...
- **Fixtures/env:** Stubbed `report-test-cycle.sh` capturing `start` and `complete` calls; assertions for Stage 4/UAT flags, stable idempotency, failed remediation reference, submit-only messaging, and sync into generated consumer `scripts/`.
