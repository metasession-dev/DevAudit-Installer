...
The steps below remain as the **manual reference** for cases where the script can't be used (e.g. no network access to DevAudit at setup time, or an unsupported stack/host combination). To reach the same end state as the script, the manual path must **copy the canonical template files from the DevAudit-Installer repo verbatim** (`sdlc/files/...`) and run `sync-sdlc.sh` to fill placeholders — do not retype workflows by hand from the snippets in this guide, as a hand-authored `ci.yml` routinely omits the DevAudit `register-release` / `upload-evidence` jobs and leaves releases unable to pass the compliance gates.

> **Note on "devaudit" paths:** throughout this guide, `devaudit/sdlc/files/...` or `path/to/devaudit/...` means a local clone of the **[DevAudit-Installer](https://github.com/metasession-dev/DevAudit-Installer)** repo — *not* the META-COMPLY portal application. The portal repo does not carry these templates.
...
> **Do not hand-author `ci.yml`.** Use the canonical template — it is the load-bearing
> path for compliance gates and drifts the moment it is retyped. The fast path
> (`sdlc-onboard.sh` → `sync-sdlc.sh`) installs it for you. Only follow the manual
> copy below if the script genuinely can't run.

The canonical workflow lives in **this repo (DevAudit-Installer)** at
`sdlc/files/ci/ci.yml.template` (Node) or `sdlc/files/ci/python/ci.yml.template`
(Python). It is a single `quality-gates` job (TypeScript + SAST + dependency audit
+ E2E + build, all on every push to `develop`) **plus two DevAudit jobs that are
...
Audit / E2E Tests gates** — DevAudit derives gate pass/fail purely from uploaded
evidence categories, not from the GitHub check status. A `ci.yml` that only saves
results as `upload-artifact` artifacts (90-day retention) produces a release that
is permanently stuck in UAT review with "Missing compliance gates". This is the
single most common onboarding failure; do not reproduce it by retyping the
workflow from memory.

**Manual install (fallback only):**

```bash
# `devaudit` below = a clone of the DevAudit-Installer repo, NOT the portal app.
cp path/to/DevAudit-Installer/sdlc/files/ci/ci.yml.template .github/workflows/ci.yml
# Fill the {{PLACEHOLDERS}} from sdlc-config.json:
bash path/to/DevAudit-Installer/scripts/sync-sdlc.sh .
```

`sync-sdlc.sh` substitutes `{{RUNNER}}`, `{{NODE_VERSION}}`, `{{SOURCE_DIRS}}`,
`{{DATABASE_*}}`, `{{APP_ENV}}`, `{{BUILD_ENV}}`, `{{E2E_*}}`, `{{PROJECT_SLUG}}`,
etc. from `sdlc-config.json`. Populate that config first — leaving `app_env` /
`build_env` empty yields a `ci.yml` with no app environment, which fails the E2E
and build steps for any app that needs runtime secrets (e.g. Supabase URLs).

### Customization Checklist

Customisation lives in **`sdlc-config.json`**, not in `ci.yml` (which is generated).
Confirm before the first sync:
...
Once all checks pass, go back to GitHub → Settings → Branches → `main` protection rules and add the required status check. The canonical template runs a single consolidated job, so add **`Quality Gates`** (and **`Check UAT Approval`** once that workflow is in place — see Additional Workflows below).
...
These are installed for you by `sdlc-onboard.sh`. If installing manually, copy the templates from this repo's `sdlc/files/ci/` (DevAudit-Installer) into your project's `.github/workflows/` and run `sync-sdlc.sh` to fill placeholders:
...
# path/to/DevAudit-Installer = a clone of the DevAudit-Installer repo, not the portal app.
# Hooks are stack-specific; swap `node` for `python` on a Python project.
