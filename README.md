...
> **DevAudit is more than a self-hosted compliance evidence portal: it applies an SDLC designed to produce reviewable evidence for ISO 29119, ISO 27001, SOC 2, GDPR, and the EU AI Act.** It helps teams implement and demonstrate controls consistently; it does not make a project compliant merely by being installed.
...
The CLI starts in seconds; the full 11-step operator flow usually takes about 5-10 minutes. It validates the PAT, detects your stack (Node / Python) + host (Railway), creates the project + a CI API key on the portal, sets GitHub secrets and branch protection, bootstraps husky + commitlint hooks, and syncs the framework templates. The CLI leaves the tree dirty for you to review + open an onboarding PR.

**What you commit afterwards:** `sdlc-config.json`, `.github/workflows/*`, `SDLC/`, `.claude/skills/*`, `scripts/`, root rule files (`AGENTS.md` / `CLAUDE.md` / `.cursorrules` / `.windsurfrules` / `GEMINI.md` / `INSTRUCTIONS.md`). First push to develop runs the gates; the portal starts seeing evidence on the same push.
...
`devaudit install` writes drop-in rule files for agents that have a native rule-file mechanism, plus the canonical `INSTRUCTIONS.md` that any LLM-driven agent can consume:
...
| **Codex / AGENTS-compatible agents**                  | `AGENTS.md` (pointer → `INSTRUCTIONS.md` + relevant `SDLC/` workflow files)                                                                     | Native root entrypoint for agents that read the AGENTS convention.                                                                                              |
...
The **`sdlc-implementer`** skill orchestrates one GitHub issue through Stages 1–5 on Claude Code, pausing at required human approvals, risk decisions, and blockers it cannot safely resolve. It delegates to five sibling specialists: **`e2e-test-engineer`** for the E2E test pack, **`governance-doc-author`** for Tier-1/2 governance docs, and the SoT-alignment family (**`requirements-aligner`**, **`adr-author`**, **`risk-register-keeper`**) at Stage 1 plan approval and Stage 3 evidence compilation. Each SoT-alignment skill maintains one persistent source-of-truth document (`docs/SRS.md`, `docs/ADR/`, `compliance/risk-register.md`) and produces the applicable per-REQ Tier 3 evidence. On Cursor / Windsurf / Gemini CLI / Codex / Copilot the same workflow runs step-by-step against the synced `INSTRUCTIONS.md`; the control gates and evidence contract stay the same even where agent integration is less automated.
